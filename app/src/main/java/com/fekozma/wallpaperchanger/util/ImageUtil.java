...
public class ImageUtil {
	...
	private static final String wallpaperFolder = "wallpapers";

	public static Optional<String> saveImageToAppstorage(Uri sourceUri, CoordinatorLayout snackbar) {
		File destinationDir = new File(ContextUtil.getContext().getFilesDir(), wallpaperFolder);
		...
		try (InputStream in = ContextUtil.getContext().getContentResolver().openInputStream(sourceUri);
			...
			DBLog.db.addLog(DBLog.LEVELS.DEBUG, "Image saving failed: " + e.getMessage(), e);
